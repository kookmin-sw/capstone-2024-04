...
class Pipeline:
    ...
    def run(self, video_path, visualize = False) -> List[INFO]:
        ...
        if visualize:
            ...
            out = cv2.VideoWriter('out-vis_input_video_merged.mp4', fourcc, fps, (width, height)) # TODO: 저장 경로 설정
        ...
        with open('resluts_for_eval.json', 'w') as f:
            json.dump(reslut_dict, f, indent=4)

        return info_list
    
if __name__ == '__main__':
    pipeline = Pipeline()
    ret = pipeline.run('colab_output_video2.mp4', visualize=True)
    print(ret)
